...
> **V 0.2 Beta** · 基于 Three.js 的 3D 第一人称射击游戏
> （v0.2 新增：连杀评级、局内强化与敌人成长、护甲道具、技能面板、四场景 BGM 切换体系）
> 零构建、零依赖、单页面启动、离线可玩 —— 双击 `index.html` 开打！

![version](https://img.shields.io/badge/version-V0.2%20beta-ffd76e)
...
配合 **12 秒连杀评级**（D→C→B→A→S→SS→SSS）、**护甲道具**（15 层封顶，每层减 2 伤）、
...
- **护甲道具**：每 10 秒 25% 概率生成（生成时屏幕提示），拾取 +2 防御（≤15 层，满层停止生成；
  每层减免 2 点所受伤害；小地图显示为青色圆帽淡蓝描边图标）
...
├── images/               实机截图
└── tests/
    ├── smoke-test.js     无头冒烟测试（node tests/smoke-test.js，120 项）
...
```bash
cd 蓝色肥鱼大作战
node tests/smoke-test.js    # 120 项无头冒烟测试
